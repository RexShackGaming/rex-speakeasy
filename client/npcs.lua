...
CreateThread(function()
    while true do
        Citizen.Wait(500)
        for k,v in pairs(Config.SpeakeasyNPCs) do
            local playerCoords = GetEntityCoords(PlayerPedId())
            local distance = #(playerCoords - v.npccoords.xyz)

            if distance < Config.DistanceSpawn and not spawnedPeds[k] then
                local spawnedPed = NearPed(v.npcmodel, v.npccoords, v.speakeasyid, v.menutype)
...
function NearPed(npcmodel, npccoords, speakeasyid, menutype)
    ...
    exports.ox_target:addLocalEntity(spawnedPed, {
        {
            ...
            onSelect = function()
                TriggerEvent('rex-speakeasy:client:'..menutype..'menu', speakeasyid)
            end,
            distance = 3.0
        }
    })
